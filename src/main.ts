...
export type CarveViewMode = 'preview' | 'source' | 'split'
...
export default class CarvePlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    for (const [id, name, mode] of [['carve-reading-view', 'Open reading view', 'preview'], ['carve-source-view', 'Open source view', 'source'], ['carve-split-view', 'Open live split view', 'split']] as const) this.addCommand({ id, name, checkCallback: (checking) => { const view = this.app.workspace.getActiveViewOfType(CarveView); if (!view) return false; if (!checking) view.setMode(mode); return true } })
